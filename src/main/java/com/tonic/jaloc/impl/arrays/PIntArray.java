...
public final class PIntArray extends AbstractPrimitiveArray<PIntWriter>
{
     ...
     */
    public static PIntArray create(Path path, long length)
    {
        return new PIntArray(FileMappedAllocator.create(path, ElementSize.DWORD.byteSize(MappedArrays.requireLength(length))), length);
    }
     ...
     * @throws UnsupportedOperationException if file mapping is unavailable on this JVM
     */
    public static PIntArray open(Path path)
    {
        FileMappedAllocator allocator = FileMappedAllocator.open(path);
        return new PIntArray(allocator, MappedArrays.elementCount(allocator.fileBytes(), ElementSize.DWORD.getSize()));
    }
