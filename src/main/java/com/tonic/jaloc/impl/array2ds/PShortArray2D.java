...
/**
 * A fixed-shape 2D short array over a single native allocation, addressed row-major.
 */
public final class PShortArray2D extends AbstractNativeArray2D
{
     ...
     */
    public PShortArray2D(long rows, long columns)
    {
     ...
     * @throws IllegalArgumentException if either dimension is negative
     */
    public PShortArray2D(NativeAllocator allocator, long rows, long columns)
    {
        super(rows, columns);

        this.array = new PShortArray(allocator, Math.multiplyExact(rows, columns));
    }
     ...
     */
    public short get(long row, long column)
    {
     ...
     * @throws IllegalStateException if closed
     */
    public void set(long row, long column, short value)
    {
