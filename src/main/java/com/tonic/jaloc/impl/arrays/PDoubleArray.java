...
public final class PDoubleArray extends AbstractPrimitiveArray<PDoubleWriter>
{
     ...
     */
    public static PDoubleArray create(Path path, long length)
    {
        return new PDoubleArray(FileMappedAllocator.create(path, ElementSize.QWORD.byteSize(MappedArrays.requireLength(length))), length);
    }
     ...
     * @throws UnsupportedOperationException if file mapping is unavailable on this JVM
     */
    public static PDoubleArray open(Path path)
    {
        FileMappedAllocator allocator = FileMappedAllocator.open(path);
        return new PDoubleArray(allocator, MappedArrays.elementCount(allocator.fileBytes(), ElementSize.QWORD.getSize()));
    }
