...
/**
 * A fixed-shape 2D double array over a single native allocation, addressed row-major.
 */
public final class PDoubleArray2D extends AbstractNativeArray2D
{
     ...
     */
    public PDoubleArray2D(long rows, long columns)
    {
     ...
     * @throws IllegalArgumentException if either dimension is negative
     */
    public PDoubleArray2D(NativeAllocator allocator, long rows, long columns)
    {
        super(rows, columns);

        this.array = new PDoubleArray(allocator, Math.multiplyExact(rows, columns));
    }
